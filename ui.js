...
function switchRightPane(tab) {
    const aiContent = document.getElementById('aiPaneContent');
    const finalizeContent = document.getElementById('finalizePaneContent');
    const tabAI = document.getElementById('tabAI');
    const tabFinalize = document.getElementById('tabFinalize');
    const aiToggleBtn = document.getElementById('aiToggleBtn');
    const finalizeToggleBtn = document.getElementById('finalizeToggleBtn');

    if (!aiContent || !finalizeContent) return;

    if (tab === 'ai') {
        aiContent.style.display = '';
        finalizeContent.style.display = 'none';
        tabAI.classList.add('active');
        tabFinalize.classList.remove('active');
        if (aiToggleBtn) aiToggleBtn.classList.add('active');
        if (finalizeToggleBtn) finalizeToggleBtn.classList.remove('active');
    } else if (tab === 'finalize') {
        aiContent.style.display = 'none';
        finalizeContent.style.display = '';
        tabFinalize.classList.add('active');
        tabAI.classList.remove('active');
        if (finalizeToggleBtn) finalizeToggleBtn.classList.add('active');
        if (aiToggleBtn) aiToggleBtn.classList.remove('active');
    }
}

function selectRightPane(tab) {
    DataCore.setUI('rightPaneTab', tab); 
    switchRightPane(tab);
}

// 切换AI面板（现在改为切换到AI Tab）
function toggleAIPanel() {
    const rightPane = document.getElementById('rightPane');
    const btn = document.getElementById('aiToggleBtn');
    if (!rightPane || !btn) return;
...
function initResizeHandles() {
    ...
    function restoreWidths() {
        ...
        if (aw && rightPane) { rightPane.style.width = aw + 'px'; rightPane.style.minWidth = aw + 'px'; }
    }
    restoreWidths();
    ...
    if (rightHandle && rightPane) createResize(rightHandle, rightPane, 'right');
}
